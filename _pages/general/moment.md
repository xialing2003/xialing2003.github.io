...
After some  linear transformation, the moment tensor matrix can be transformed into a diagonal form. Then the three eigenvalues can be thought of as coming from three contributions, $$M = M_{iso} + M_{DC} + M_{CLVD}$$ (CLVD means Compensated Linear Vector Dipole). For $$M_{iso}$$, the three eigenvalues are equal, which quantify the isotropic volumetric changes. $$M_{DC}$$ describes the pure-shear slip, with one eigenvalue equal to 0 and the other two opposite values. $$M_{CLVD}$$ represents a compensated linear vector dipole, characterized by eigenvalues in the ratio $(-1, -1, 2)$ such that the largest eigenvalue in absolute value is exactly twice the other two. $M_{CLVD}$ is often associated with non-planar faulting, tensile crack opening, or fluid-driven source processes such as those observed in volcanic environments. 
